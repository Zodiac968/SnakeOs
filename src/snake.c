...
VOID snakeInitialise(UINT32 *fb, UINTN w, UINTN h) {
        ...
        for (UINTN i = 0; i < 100; i++) {
                if (i < snakeLength) snakePos[i] = 5 + i;
                else snakePos[i] = 0;
        }
...
BOOLEAN snakeUpdate(EFI_SIMPLE_TEXT_INPUT_PROTOCOL *ci) {
        ...
        if (xvel == 1 && currentCol == snakeCols - 1) return FALSE;
        if (xvel == -1 && currentCol == 0) return FALSE;
        if (yvel == 1 && currentRow == snakeRows - 1) return FALSE;
        if (yvel == -1 && currentRow == 0) return FALSE;
